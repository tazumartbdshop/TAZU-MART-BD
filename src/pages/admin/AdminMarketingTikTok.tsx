...
export default function AdminMarketingTikTok() {
  ...
  useEffect(() => {
    fetch('/api/admin/marketing/config?tableName=tiktok_settings&rowId=tiktok_config')
      .then(res => res.json())
      .then(data => {
  ...
  const handleSave = async () => {
    ...
    setSaving(true);
    try {
      const res = await fetch('/api/admin/marketing/schema-check?tableName=tiktok_settings');
      const data = await res.json();
      if (data.status === 'success') {
        const isMissing = !data.schemaState.tiktok_settings?.exists || data.schemaState.tiktok_settings?.missingColumns.length > 0;
        if (isMissing) {
        ...
        <div className="fixed inset-0 bg-black/60 flex items-center justify-center p-4 z-50 backdrop-blur-sm">
          <div className="bg-white p-8 max-w-md w-full border border-zinc-200 shadow-2xl">
            ...
            <p className="text-sm text-zinc-600 mb-6">
              The TikTok settings table or required columns are missing in your Supabase database.
            </p>
