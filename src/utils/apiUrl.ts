...
export function getApiUrl(path: string): string {
  // In AI Studio / Cloud Run containers, we should ALWAYS use relative paths 
  // for our own backend API to avoid CORS and "Failed to fetch" unreachable absolute URL errors.
  // The absolute prodBackend URL in VITE_API_BASE_URL should only be used 
  // if the app is hosted on an EXTERNAL STATIC DOMAIN (like Hostinger).
  
  if (typeof window !== 'undefined') {
    const hostname = window.location.hostname;
    // If we are on our own server (preview or prod container), relative paths are best.
    if (
  ...
  // Production Cloud Run active server domain fallback for external hosting.
  const prodBackend = import.meta.env.VITE_API_BASE_URL || '';
